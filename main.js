define(function (require, exports, module) {
  ...
  const bracketsJasmineDomain = new NodeDomain(
    ...
    ExtensionUtils.getModulePath(module, "src/node/domain")
  );
  ...
  const handleLinterAsync = (text, filePath) => {
    ...
    isWorking = true;
    updateStatus();
    bracketsJasmineDomain.exec("runTests", params).then(
      function (result) {
        isWorking = false;
        updateStatus();
        ...
        try {
          if (window.bracketsInspectionGutters) {
            ...
            log.error(
              `No bracketsInspectionGutters found on window, gutters disabled.`
            );
          }
        } catch (e) {
          console.error(log(e));
